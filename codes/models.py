...
class KGEModel(nn.Module, ABC):
    ...
    def forward(self, sample, batch_type=BatchType.SINGLE):
        ...
        Args:
            ...
            sample: different format for different batch types.
                - SINGLE: tensor with shape [batch_size, 3]  --> positive만 들어옴
    ...
    def train_step(model, optimizer, train_iterator, args):
        ...
        # pos_triple : (head, rel, tail)
        # neg_triples : (ent, ent, ent, ...)
        # subsampling_weight : (1/(head의 degree + tail의 degree))^0.5
        # self.batch_type : (head, rel, tail)
...
class HAKE(KGEModel):
    def __init__(self, num_entity, num_relation, hidden_dim, gamma, modulus_weight=1.0, phase_weight=0.5):
        ...
        self.num_entity = num_entity
        self.num_relation = num_relation
        self.hidden_dim = hidden_dim # 500
        self.epsilon = 2.0
...
#         print("gamma: ", self.gamma) # 6
#         print("epsilon: ", self.epsilon) # 2
        self.embedding_range = nn.Parameter(
            torch.Tensor([(self.gamma.item() + self.epsilon) / hidden_dim]),
            requires_grad=False
        ) # 0.0160
#         print("self.embedding_range : ", self.embedding_range)
#         exit(True)
...
#         print("self.relation_embedding shape : ", self.relation_embedding.shape)
#         print("self.relation_embedding1 : ", self.relation_embedding)
...
#         print("self.relation_embedding2 : ", self.relation_embedding)
#         print("hidden_dim: ", hidden_dim)
        nn.init.ones_(
            tensor=self.relation_embedding[:, hidden_dim:2 * hidden_dim]
        ) # hidden_dim ~ hidden_dim*2 까지 1로 초기화
#         print("self.relation_embedding3 : ", self.relation_embedding)

        nn.init.zeros_(
            tensor=self.relation_embedding[:, 2 * hidden_dim:3 * hidden_dim]
        ) # hidden_dim * 2 ~ hidden_dim*3 까지 0로 초기화
#         print("self.relation_embedding4 : ", self.relation_embedding)
    ...
    def func(self, head, rel, tail, batch_type):
        phase_head, mod_head = torch.chunk(head, 2, dim=2) # head를 phase_head와 mod_head로 2등분
        phase_tail, mod_tail = torch.chunk(tail, 2, dim=2)
        
        
        phase_relation, mod_relation, bias_relation = torch.chunk(rel, 3, dim=2) # relation을 phase, mode, bias로 3등분

#         print(head.shape) # 512, 1 1000
#         print(rel.shape)  # 512, 1 1500
#         print(tail.shape) # 512, 1 1000
